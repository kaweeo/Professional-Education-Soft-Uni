...
for k, v in submissions.items():
    ...
    sorted_items = dict(sorted(v.items(), key=lambda x: (-x[1], x[0])))  #  for each contest print the participants ordered by points in descending order,
                                                                        # then ordered by name in ascending order.
    for k2, v2 in sorted_items.items():
        print(f'{i}. {k2} <::> {v2}')
        i += 1
print('Individual standings:')
sorted_names = dict(sorted(users.items(), key=lambda x: (-sum(x[1].values()), x[0]))) # individual statistics for every participant ordered by total points in descending order,
